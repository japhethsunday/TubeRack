 ...
 * Requires CLOUDNIVO_DATABASE_URL in the environment (see .env.example).
...
async function main() {
  const databaseUrl = process.env.CLOUDNIVO_DATABASE_URL;
  if (!databaseUrl) {
    console.error("CLOUDNIVO_DATABASE_URL is not set. Add credentials first (see .env.example).");
    process.exit(1);
  }
