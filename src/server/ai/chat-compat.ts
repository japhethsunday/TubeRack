...
async function liveModels(p: ChatProvider): Promise<Set<string> | null> {
  ...
  try {
    const res = await fetch(`${p.baseUrl}/models`, { headers: { Authorization: `Bearer ${p.key}` }, signal: AbortSignal.timeout(8_000) });
...
export async function chatGenerateText(
  ...
  const prompt = request.json ? `${request.prompt}\n\nReply with valid JSON only — no prose, no code fences.` : request.prompt;
  ...
  for (const model of await modelOrder(p)) {
    ...
    try {
      const res = await fetch(`${p.baseUrl}/chat/completions`, {
        method: "POST",
        headers: { Authorization: `Bearer ${p.key}`, "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
          model,
      ...
      if (!res.ok) {
        lastError = new Error(`${p.name} ${model} ${res.status}: ${(await res.text()).slice(0, 200)}`);
        continue;
      }
      const body = (await res.json()) as { choices?: { message?: { content?: unknown } }[] };
      const raw = body.choices?.[0]?.message?.content;
      // Some APIs return content as parts: [{type:"text", text:"…"}].
      const content = Array.isArray(raw) ? raw.map((c) => (c as { text?: string })?.text ?? "").join("") : String(raw ?? "");
  ...
  throw lastError instanceof Error ? lastError : new Error(`No ${p.name} model could answer.`);
}
