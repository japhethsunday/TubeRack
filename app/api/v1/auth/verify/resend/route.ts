...
export async function POST(request: Request) {
  try {
    ...
    `;
    __recordAttempt({ to: user.email, subject: "Verify your TubeRack email", text: "Verification link (pending email provider).", kind: "verify" });
    const result = await sendEmail({ to: user.email, subject: "Verify", text: "", kind: "verify" });
